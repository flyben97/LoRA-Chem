...
def create_instruction(description, G):
    return {
        "instruction": "What is the enantioselectivity of this reaction?",
        "input": f"{description}",
        "output": f"OK, this is a Nickel-Catalyzed Enantioselective Cross-Coupling reaction. Based on the chemical reaction information you provided, the enantioselectivity of this reaction is: {str(G)}.",
        "history": []
    }
...
for csv_file in csv_files:
    try:
        ...
        data = df[['Description', 'G']]
        
        # Convert data to JSON format
        json_data = [create_instruction(row['Description'], row['G']) for _, row in data.iterrows()]
